import {
  useState, useEffect, useMemo, Button, IconButton, TextField, Box,
...
const ModalAddFTPServer = (props) => {
  ...
  const {
    actionAddFTPServer,
    openModalAddFTPServer,
    handleCloseModalAddFTPServer,
    dataModalAddFTPServer,
  } = props;

  // Memo hoá để không đổi tham chiếu mỗi render
  ...
  useEffect(() => {
    ...
    setErrors({});
  }, [openModalAddFTPServer, dataModalAddFTPServer, actionAddFTPServer, defaultData]);
  ...
  const handleConfirm = async () => {
    ...
    let res;
    if (actionAddFTPServer === 'CREATE') {
      res = await createFTPServer(dataFTPServer);
    } else if (actionAddFTPServer === 'UPDATE') {
      res = await updateFTPServer(dataFTPServer);
    }
    if (res && res.EC === 0) {
