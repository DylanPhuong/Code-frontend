...
const ModalDatabase = (props) => {
  ...
  const { actionAddDatabase, openModalDatabase, handleCloseModalDatabase, dataModalDatabase } = props;
  ...
  useEffect(() => {
    ...
    if (
      actionAddDatabase === 'UPDATE MYSQL' ||
    ...
    setErrors({});
  }, [openModalDatabase, dataModalDatabase, actionAddDatabase, defaultData]);
  ...
  const handleTestConnect = async () => {
    ...
    if (res && res.EC === 0) {
      ...
      toast.error((res?.EM || 'Kết nối thất bại') + detail);
    }
  };
  ...
  const handleConfirm = async () => {
    if (!validateAll()) return;
    let res;
    if (actionAddDatabase === 'CREATE MYSQL') {
      res = await createMySQLServer(dataDatabase);
    } else if (actionAddDatabase === 'UPDATE MYSQL') {
      res = await updateMySQLServer(dataDatabase);
    } else if (actionAddDatabase === 'CREATE SQL') {
      res = await createSQLServer(dataDatabase);
    } else if (actionAddDatabase === 'UPDATE SQL') {
      res = await updateSQLServer(dataDatabase);
    }
