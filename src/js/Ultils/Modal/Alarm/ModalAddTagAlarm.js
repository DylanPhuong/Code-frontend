import {
  useState, useEffect, useMemo, Button, IconButton, MenuItem, TextField,
...
const ModalAddTagAlarm = (props) => {
  ...
  const defaultDataChoose = useMemo(() => ({
    tagnameId: '',
    name: '',
    deviceId: '',
    deviceName: '',
  ...
  const {
    action, openModalAddAlarm, handleCloseModalAddAlarm,
    setopenModalSearchTag, setActionChooseTag, dataModalAlarm
  } = props;
  ...
  }, [dataModalAlarm, defaultDataChoose]);
  ...
  const handleOnchangeInput = (value, name) => {
    const _dataAlarm = _.cloneDeep(dataAlarm);
  ...
  return (
    <Modal open={openModalAddAlarm} onClose={handleClose} onKeyDown={() => {}}>
      <Box sx={style}>
        {/* Header */}
        ...
        >
            ...
            InputProps={{
              ...
              endAdornment: (
                <InputAdornment position="end" sx={{ mr: 1 }}>
                  <IconButton edge="end" onClick={handleOpenModalSearch}>
                    <FindInPageIcon />
                  </IconButton>
                </InputAdornment>
              ),
          ...
          >
            <MenuItem value=">" sx={{ fontWeight: 500, fontSize: '1.5rem' }}>
              {'>'}
            </MenuItem>
            <MenuItem value="<" sx={{ fontWeight: 500, fontSize: '1.5rem' }}>
              {'<'}
          ...
          >
            <Typography
              variant="h6"
              sx={{ fontWeight: 500, textAlign: 'center', mt: 2.5, ml: -2 }}
            >
              Cảnh báo đến:
            </Typography>
