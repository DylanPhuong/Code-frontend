...
const ListHistorical = () => {
  ...
  const fetchConfig = async () => {
    const response = await fetchConfigHistorical();
    setLoading(false);
    if (response && response.EC === 0 && Array.isArray(response.DT?.DT)) {
      const rows = response.DT.DT.map((item) => ({
        id: item._id,
  ...
  const conformDeleteHistorical = async () => {
    const res = await deleteHistorical({ list: dataModalDelete });
        ...
        <>
          ...
          >
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                handleShowModalEditTag(params.row);
              }}
            >
              Sửa
            </Button>
            ...
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                handleDeleteHistorical(params.row);
              }}
            >
              Xóa
            </Button>
