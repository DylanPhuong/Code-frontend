...
//             const protocol = response.DT.Protocol?.map(item => ({ id: item._id, name: item.name })) || [];
//             const modbus = response.DT.Modbus?.map(item => ({ id: item._id, name: item.name })) || [];
//             const siemens = response.DT.Siemens?.map(item => ({ id: item._id, name: item.name })) || [];
//             const mqtt = response.DT.MQTT?.map(item => ({ id: item._id, name: item.name })) || [];
...
//                 const com = Array.isArray(listComs) ? listComs.find(c => c.serialPort === serialPort) : null;
...
//                             sx={{ mr: 1, textTransform: 'none', minWidth: 76 }}
...
//                             sx={{ textTransform: 'none', minWidth: 76 }}
...
const ListDevices = () => {
    ...
    const [actionModalDevice, setactionModalDevice] =
        useState("CREATE");
    const [actionDeleteDevice, setactionDeleteDevice] =
        useState("");

    const [isShowModalDelete, setisShowModalDelete] =
        useState(false);
    const [isShowModalProtocol, setisShowModalProtocol] =
        useState(false);
    const [isShowModalDevice, setisShowModalDevice] =
        useState(false);
    ...
    const handleCloseModalDelete = () => setisShowModalDelete(false);
    const handleCloseModalProtocol = () =>
        setisShowModalProtocol(false);
    ...
    const columns = [
        ...
        },
        {
            field: "action",
            headerName: "Action",
            width: 170,          // ✅ cột Action rộng cố định
            minWidth: 170,
            headerAlign: "center",
            align: "center",
            sortable: false,
            filterable: false,
            disableColumnMenu: true, // ✅ ẩn nút "..." ở header
            renderCell: (params) => {
                if (!params || !params.row) return null;
                return (
                    <Box
