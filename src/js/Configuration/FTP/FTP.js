...
const ListFTP = () => {
    const [actionAddFTPServer, setActionAddFTPServer] =
        useState("");
    const [openModalAddFTPServer, setOpenModalAddFTPServer] =
        useState(false);
    const [dataModalAddFTPServer, setDataModalAddFTPServer] =
        useState(null);
    ...
    const [dataModalDelete, setDataModalDelete] =
        useState(null);
    const [actionDeleteFTPServer, setActionDeleteFTPServer] =
        useState("");
    ...
    useEffect(() => {
        ...
        socket.on("SERVER SYNC RESULT", (res) => {
            if (res && res.EC === 0) {
                ...
                toast.error(res?.EM || "Sync lỗi");
            }
        });

        return () => {
            socket.off("SERVER SYNC RESULT");
        };
    }, []);
    ...
    const confirmDeleteFTPServer = async () => {
        if (!dataModalDelete || dataModalDelete.length === 0)
            return;
        const res = await deleteFTPServer({
