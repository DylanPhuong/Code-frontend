...
const ModalDevice = (props) => {
  ...
  // defaultData cần ổn định tham chiếu để dùng trong deps
  ...
  }, [isShowModalDevice, action, dataModalDevice, defaultData]);
  ...
  const handleOnchangeInput = (value, name) => {
    // Khi đổi Protocol → reset các trường liên quan
    if (name === 'protocol') {
      setDataDevice((prev) => {
        const updated = {
          ...prev,
    ...
    if (name === 'driverName') {
      setDataDevice((prev) => {
        const updated = { ...prev, driverName: value };
        switch (value) {
  ...
  const validateAll = () => {
    ...
    const fieldsToValidate = ['name', 'protocol', 'timeOut'];
  ...
  const handleConfirmDevice = async () => {
    ...
    const res = action === 'CREATE' ? await createNewDevice(dataDevice) : await updateCurrentDevice(dataToUpdate);
    if (res && res.EC === 0) {
