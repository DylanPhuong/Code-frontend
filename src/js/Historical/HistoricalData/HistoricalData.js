...
const ListHistorical = () => {
    ...
    const fetchHistorialValue = async () => {
        setLoading(true);
        const response = await fetchAllHistoricalValue();
        if (response && response.EC === 0 && response.DT?.DT) {
            const records = [];
            response.DT.DT.forEach((dayItem) => {
                dayItem.values.forEach((v) => {
                    Object.keys(v.value).forEach((tagKey) => {
                        const tag = v.value[tagKey];
                        let string_status;
                        if (tag.status === 1) string_status = "Normal";
                        else if (tag.status === 2) string_status = "Over range";
                        else if (tag.status === 3) string_status = "Disconnect";
                ...
                response.DT.forEach((item) => {
                    if (item.value) {
                        ...
                        dataFinds.push({
                            ...
                            unit: tag.unit,
                            status: string_status,
                            timestamp: dayjs(item.ts).format(
                                "DD-MM-YYYY - HH : mm : ss"
                            ),
                        });
                    }
    ...
    const handleExportCSV = () => {
        ...
        const tagnamedownload = selectedTagObj
            ? selectedTagObj.name
            : "historical_data";
    ...
    return (
        ...
        >
            ...
            >
                <Stack
                    direction="row"
                    spacing={1.5} // độ rộng name tìm kiếm
                    alignItems="center"
                    sx={{
                        height: "100%",
