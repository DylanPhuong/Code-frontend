...
const ListAlarm = () => {
    ...
    const fetchTagAlarm = async () => {
        const response = await fetchAllTagAlarm();
        if (response && response.EC === 0 && response.DT?.DT) {
            const tagAlarm = response.DT.DT.map((item) => ({
                id: item._id,
    ...
    const fetchHistorialValue = async () => {
        setLoading(true);
        const response = await fetchAllAlarmValue();
        if (response && response.EC === 0 && response.DT?.DT) {
            const rowsWithId = [];
            let idCounter = 1;
                ...
                dayItem.alarms.forEach((alarmGroup) => {
                    alarmGroup.alarm.forEach((alarmItem) => {
                        rowsWithId.push({
                            ...
                            type: alarmItem.type,
                            timestamp: dayjs(alarmGroup.ts).format(
                                "DD/MM/YYYY - HH:mm:ss"
                            ),
                        });
                    });
    ...
    const handleFindAlarm = async () => {
        ...
        try {
            ...
            const selectedTagObj = listTagAlarm.find(
                (tag) => tag.tagnameId === selectedTag
            );
            const tagNameId = selectedTagObj ? selectedTagObj.tagnameId : "";

            const response = await findAlarmTime({
                startTime,
                endTime,
                tagNameId,
            });
