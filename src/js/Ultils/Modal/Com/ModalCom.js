...
const ModalCom = (props) => {
  ...
  // ổn định hoá defaultData để không đổi tham chiếu mỗi render
  ...
  useEffect(() => {
    if (isShowModalCom) {
      ...
      // dùng defaultData đã memo hoá
      setdataComs(defaultData);
      setOriginalData(defaultData);
    }
  }, [isShowModalCom, dataModalCom, defaultData]);
  ...
  const handleOnchangeInput = (value, name) => {
    const _dataCom = _.cloneDeep(dataCom);
    _dataCom[name] = value;
    setdataComs((prev) => ({
      ...prev,
      [name]: value,
  ...
  const handleConfirmCom = async () => {
    ...
    const res = await updateCurrentCom(dataToUpdate);
    if (res && res.EC === 0) {
